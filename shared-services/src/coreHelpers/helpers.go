...
func BuildJSONReply(reply interface{}, requestorId, message string) (jsonReply []byte) {
	...
	if jsonReply, errorInfo.Error = json.Marshal(reply); errorInfo.Error != nil {
		...
		errorInfo.Error = coreError.ErrJSONGenrationFailed
...
// RedirectLogOutput - Not going to test
func RedirectLogOutput(logDirectory string) (logFileHandlerPtr *os.File, logFQN string) {
	...
	tLogFileName = strings.Replace(strings.Replace(strings.Replace(tDateTime, " ", "_", -1), ":", "_", -1), ".", "_", -1)
	logFQN = logDirectory + "/" + tLogFileName

	// Set log file output
	logFileHandlerPtr, err = os.OpenFile(logFQN, os.O_RDWR|os.O_CREATE|os.O_APPEND, 0666)
	if err != nil {
		log.Fatalf("Error opening %v file: %v%v", tLogFileName, err, constants.ENDING_EXECUTION)
	}

	log.Printf("The log file has been redirected to %v.", logFQN)
	log.SetOutput(logFileHandlerPtr)
	coreError.SetOutputMode(constants.OUTPUT_LOG)

	return
}

// RemovePidFile
func RemovePidFile(pidFQN string) (errorInfo coreError.ErrorInfo) {
...
// SendReply
func SendReply(functionName string, jsonReply []byte, msg *nats.Msg) (errorInfo coreError.ErrorInfo) {
...
func UnmarshalMessageData(msg *nats.Msg, requestPtr any) (errorInfo coreError.ErrorInfo) {
...
// WriteFile
func WriteFile(fullQualifiedName string, fileData []byte, filePermissions os.FileMode) (errorInfo coreError.ErrorInfo) {
...
// WritePidFile
func WritePidFile(directory string) (errorInfo coreError.ErrorInfo) {
